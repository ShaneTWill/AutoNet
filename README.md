...
For using the model after training you will have to first read in and execute the model file. from there to reinstate the weights and biases you run the line below.
	
	saver.restore(sess,’absolute path’)

The absolute path is the path to the checkpoint file that you wish to restore with. After the checkpoint has been loaded in and the variables have been restored you are ready to make predictions. To make a prediction you just need to call the model while passing the inputs as a numpy array of floats. A call would look like this.
